...
class BleService {
  ...
  final Map<String, epi.Beacon> beacons = {
    'E4:E1:12:9A:49:C3':
        epi.RealBeacon('E4:E1:12:9A:49:C3', 'blukii', Point(0, 0)),
    'E4:E1:12:9A:4A:03':
        epi.RealBeacon('E4:E1:12:9A:4A:03', 'blukii', Point(2.40, 0)),
    'E4:E1:12:9A:4A:0F':
        epi.RealBeacon('E4:E1:12:9A:4A:0F', 'blukii', Point(3.90, 2.35)),
    'E4:E1:12:9B:0B:98':
        epi.RealBeacon('E4:E1:12:9B:0B:98', 'blukii', Point(2.7, 4.60)),
    'E4:E1:12:9A:49:EB':
        epi.RealBeacon('E4:E1:12:9A:49:EB', 'blukii', Point(0, 4.6))
  };
  ...
  BleService({this.onLocationServicesDisabled, this.onBluetoothDisabled}) {
    ...
    //Initialize filter
    Filter filter = SimpleUKF(4, 2, 5, hxUserLocation, fxUserLocation,
  ...
  void startPositioning() async {
    ...
    rangingStream = flutterBeacon.ranging([region]).listen(_updateBeacon);
  }
  ...
  void _updateBeacon(RangingResult result) {
    ...
    for (var beacon in result.beacons) {
      if (beacon.macAddress == null ||
          !beacons.containsKey(beacon.macAddress!)) {
        continue;
      }
      foundBeacons.update(beacon.macAddress.toString(), (value) {
        value.rssiUpdate(beacon.rssi);
        value.reset();
        return value;
      },
          ifAbsent: () => ReceivedBeacon(beacons[beacon.macAddress!]!,
              (b) => foundBeacons.remove(b.beacon.id)));
      updated = true;
    }
    ...
    if (updated) {
      if (foundBeacons.length >= 3) {
        var beaconList =
            foundBeacons.entries.map((k) => k.value.beacon).toList();
  ...
  void setBeacons(List<Beacon> beacons) {
    ...
    var b = epi.RealBeacon(
        beacon.id, beacon.id, Point(0, 0, beacon.position.altitude));
...
class CoordinationSystemConverter {
  ...
  Pos cartesianToGeographic(Point p) {
    ...
    var userPos =
        Pos(origin.lat + lon_delta, origin.lon + lat_delta, altitude: p.z);
  ...
  double distance(Pos p1, Pos p2) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      cos_2_sigma_m =
          cos(sigma) - ((2 * sin(u1) * sin(u2)) / cos_alpha_squared);
          ...
          (1 - C) *
              ...
              (sigma +
                  C *
                      sin_sigma *
                      (cos_2_sigma_m +
                          C * cos_sigma * (-1 + 2 * pow(cos_2_sigma_m, 2))));
    }
    ...
    var delta_sigma = B *
        ...
        (cos_2_sigma_m +
            (0.25 *
                B *
                (cos_sigma * (-1 + 2 * pow(cos_2_sigma_m, 2)) -
                    ((B / 6) *
                        cos_2_sigma_m *
                        (-3 + 4 * pow(sin_sigma, 2)) *
                        (-3 + 4 * pow(cos_2_sigma_m, 2))))));
